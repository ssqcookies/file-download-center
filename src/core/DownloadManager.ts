...
export class DownloadManager {
    ...
    return new Promise<void>((resolve) => {
      ...
      for (let i = 0; i < totalChunks; i++) {
        ...
        queue.add(async () => {
          ...
          try {
            const downloadedChunk = await this.retryPolicy.execute(() =>
              this.chunkDownloader.download(task.url, chunk),
            );
   ...
   * 注意：由于 ChunkStore 的 BUG，getDownloadedChunkIndices 返回空数组，
   * 导致所有分片被重新下载。
   */
  async resume(taskId: string): Promise<void> {
    ...
    // 获取已下载的分片序号（BUG：返回空数组）
    const downloadedIndices =
      await this.chunkStore.getDownloadedChunkIndices(taskId);
    ...
    const totalChunks = task.totalChunks || Math.ceil(task.fileSize / chunkSize);
    const tracker =
      this.trackers.get(taskId) || new ProgressTracker(taskId, task.fileSize);
